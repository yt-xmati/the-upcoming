...
class MainMenuState extends MusicBeatState
{
	public static var psychEngineVersion:String = '0.5.1'; //This is also used for Discord RPC
		public static var funnyVersion:String = '1.5'; //This is also used for Discord RPC
	...
	override function create()
	{
		...
		var yScroll:Float = Math.max(0 - (0 * (optionShit.length - 5)), 0);
		...
		{
			var offset:Float = 50 - (Math.max(optionShit.length, 1) - 5) * 160;
		...
		versionShit.setFormat("Comic Sans MS Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, ClientPrefs.getResolution()[1] - 64, 0, "Dave And Bambi Interdimensional Version v" + funnyVersion, 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("Comic Sans MS Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{
					CoolUtil.browserLoad('https://discord.gg/MpJ95CnXtb');
				}
				else
				{
					selectedSomethin = true;
